...
function run_LINSIGHT_annotation {
    local invcf=$1
    local outdir=$(dirname ${invcf})
    local outvcf=${outdir}/b37-LINSIGHT-annotation.vcf.gz

    if [[ -e "${outvcf}" ]]; then
        log "shortcutting run_LINSIGHT_annotation"
        echo ${outvcf}
        return 0;
    fi
    ...
    local hdr_msg="LINSIGHT score"

    local cmd1="
    ${BCFTOOLS} annotate \
        -a ${anno_vcf} \
        -c ${annotations} \
        -h <(echo '##INFO=<ID=LINSIGHT,Number=1,Type=Float,Description="${hdr_msg}">') \
        -O z \
        -o ${tmpvcf} \
        ${invcf} \
    "
...
function integrate_b37_annotations_to_b38 {
    ...
    local outvcf=${outdir}/b38.LINSIGHT.nosamples.vcf.gz
...
function annotate_vcf {
    ...
    log "Entering run_LINSIGHT_annotation"
    local b37_anno_vcf=$(run_LINSIGHT_annotation ${grc37_vcf})
    log "Entering integrate b37 cadd annotations back to b38"
    local b38_anno_vcf=$(integrate_b37_annotations_to_b38 ${integrate_script} ${b37_anno_vcf} ${b38_invcf_no_samples} 'LINSIGHT')
