...
@click.option('--skip-confirm', default=False, is_flag=True,
              help='Do not prompt when resuming or restarting a pipeline [default=False]')
def postvqsr3(job_db, input_vcfs, project_name, email, workspace, drm, restart, docker, skip_confirm):
    from yaps2.pipelines.postvqsr3 import Config, Pipeline
    config = Config(job_db, input_vcfs, project_name, email, workspace, docker)
    workflow = Pipeline(config, drm, restart, skip_confirm)
    workflow.run()
...
@click.option('--skip-confirm', default=False, is_flag=True,
              help='Do not prompt when resuming or restarting a pipeline [default=False]')
def postvqsr4(job_db, input_vcfs, project_name, email, workspace, drm, restart, docker, skip_confirm):
    from yaps2.pipelines.postvqsr4 import Config, Pipeline
    config = Config(job_db, input_vcfs, project_name, email, workspace, docker)
    workflow = Pipeline(config, drm, restart, skip_confirm)
    workflow.run()
...
@click.option('--skip-confirm', default=False, is_flag=True,
              help='Do not prompt when resuming or restarting a pipeline [default=False]')
def postvqsr5(job_db, input_vcfs, project_name, email, workspace, drm, restart, docker, skip_confirm):
    from yaps2.pipelines.postvqsr5 import Config, Pipeline
    config = Config(job_db, input_vcfs, project_name, email, workspace, docker)
    workflow = Pipeline(config, drm, restart, skip_confirm)
    workflow.run()
...
def postvqsr9(job_db, input_vcfs, project_name, email, workspace, drm, restart, docker, skip_confirm, task_flush):
    from yaps2.pipelines.postvqsr9 import Config, Pipeline
    config = Config(job_db, input_vcfs, project_name, email, workspace, docker)
    workflow = Pipeline(config, drm, restart, skip_confirm)
