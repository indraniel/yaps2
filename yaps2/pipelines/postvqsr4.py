...
#        bcftools_stats_summary_task = self.create_bcftools_stats_summary_task(bcftools_stats_tasks)
    ...
    def create_filter_variant_missingness_tasks(self):
        tasks = []
        stage = '4b-filter-variant-missingness'
        basedir = os.path.join(self.config.rootdir, stage)
...
def filter_variant_missingness(in_vcf, in_chrom, out_vcf, out_stats, out_log):
    args = locals()
    default = {
        'script' : pkg_resources.resource_filename('yaps2', 'resources/postvqsr/filter-missingness-sites-v2.sh'),
        'python_script' : pkg_resources.resource_filename('yaps2', 'resources/postvqsr/filter-site-missingness-v2.py'),
